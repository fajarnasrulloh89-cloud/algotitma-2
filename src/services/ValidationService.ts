...
export class ValidationService {
  ...
  private static readonly nimRegex = /^\d{6,15}$/;

  // Password minimal 8 karakter, ada huruf besar, huruf kecil, angka, dan simbol.
  private static readonly passwordRegex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z\d]).{8,}$/;

  // Nama boleh huruf, spasi, titik, dan apostrof.
  private static readonly nameRegex = /^[A-Za-zÀ-ž.'\s]{3,60}$/;
  ...
  static validateNim(nim: string): boolean {
  ...
  static validateStudentPayload(student: StudentRecord): void {
